package uk.rythefirst.nihaess.games;
...
public class ChatWordGame {
	...
	public ChatWordGame() {
		...
		defWords.add("nihachu");
		defWords.add("twitch");
		defWords.add("amazon prime");
		defWords.add("fish");
		defWords.add("vikmain");
		defWords.add("fishachu");
		
		if(!(wordFile.exists())) {
			Bukkit.getLogger().log(Level.INFO, "[NihaEss] Words file not found, creating.");
			try {
				wordFile.createNewFile();
				wordCNF = YamlConfiguration.loadConfiguration(wordFile);
				wordCNF.set("words", defWords);
				wordCNF.save(wordFile);
				Bukkit.getLogger().log(Level.INFO, "[NihaEss] Words file created.");
			} catch (IOException e) {
				e.printStackTrace();
			}
		}else {
			Bukkit.getLogger().log(Level.INFO, "[NihaEss] Words file found, loading words.");
			wordCNF = YamlConfiguration.loadConfiguration(wordFile);
		}
		wordlst = wordCNF.getStringList("words");
		Bukkit.getLogger().log(Level.INFO, "[NihaEss] Words loaded.");
	}
		...
		Bukkit.getScheduler().runTaskTimer(Main.instance, new Runnable() {
			...
			public void run() {
				...
				for(Player p : Bukkit.getOnlinePlayers()) {
					...
					Messages.sendCenteredMessage(p, ChatColor.GOLD + "Wins $5000");
				...
				new BukkitRunnable() {
					...
					public void run() {
						...
						if(!(cword.isGuessed())) {
							for(Player p : Bukkit.getOnlinePlayers()) {
								Messages.sendCenteredMessage(p, "" + ChatColor.GOLD + ChatColor.BOLD + ChatColor.UNDERLINE + "Speed Typer");
							...
							cword = null;
							Bukkit.getLogger().log(Level.INFO, "[NihaESS] Chat Word ended, the word wasn't guessed!");
						}
					}
